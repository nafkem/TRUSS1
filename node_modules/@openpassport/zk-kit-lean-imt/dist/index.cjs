...
var zkKitUtils = require('@openpassport/zk-kit-utils');
...
class LeanIMT {
    ...
    constructor(hash, leaves = []) {
        zkKitUtils.requireDefined(hash, "hash");
        zkKitUtils.requireFunction(hash, "hash");
        zkKitUtils.requireArray(leaves, "leaves");
     ...
     */
    indexOf(leaf) {
        zkKitUtils.requireDefined(leaf, "leaf");
     ...
     */
    insertMany(leaves) {
        zkKitUtils.requireDefined(leaves, "leaves");
        zkKitUtils.requireArray(leaves, "leaves");
        if (leaves.length === 0) {
    ...
    update(index, newLeaf) {
        zkKitUtils.requireDefined(index, "index");
        zkKitUtils.requireDefined(newLeaf, "newLeaf");
        zkKitUtils.requireNumber(index, "index");
        let node = newLeaf;
        for (let level = 0; level < this.depth; level += 1) {
            this._nodes[level][index] = node;
            if (index & 1) {
    ...
    updateMany(indices, leaves) {
        zkKitUtils.requireDefined(leaves, "leaves");
        zkKitUtils.requireDefined(indices, "indices");
        zkKitUtils.requireArray(leaves, "leaves");
        zkKitUtils.requireArray(indices, "indices");
        ...
        for (let i = 0; i < indices.length; i += 1) {
            zkKitUtils.requireNumber(indices[i], `index ${i}`);
    ...
    static verifyProof(proof, hash) {
        zkKitUtils.requireDefined(proof, "proof");
        const { root, leaf, siblings, index } = proof;
        zkKitUtils.requireDefined(proof.root, "proof.root");
        zkKitUtils.requireDefined(proof.leaf, "proof.leaf");
        zkKitUtils.requireDefined(proof.siblings, "proof.siblings");
        zkKitUtils.requireDefined(proof.index, "proof.index");
        zkKitUtils.requireArray(proof.siblings, "proof.siblings");
        zkKitUtils.requireNumber(proof.index, "proof.index");
        let node = leaf;
    ...
    static import(hash, nodes, map) {
        zkKitUtils.requireDefined(hash, "hash");
        zkKitUtils.requireDefined(nodes, "nodes");
        zkKitUtils.requireFunction(hash, "hash");
        zkKitUtils.requireString(nodes, "nodes");
        if (map) {
            zkKitUtils.requireDefined(map, "map");
            zkKitUtils.requireFunction(map, "map");
        }
