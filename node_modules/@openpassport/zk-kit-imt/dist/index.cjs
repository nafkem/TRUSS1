...
class IMT {
    ...
    constructor(hash, depth, zeroValue, arity = 2, leaves = []) {
        ...
        zkKitUtils.requireNumber(depth, "depth");
        zkKitUtils.requireTypes(zeroValue, "zeroValue", ["number", "string", "bigint"]);
        zkKitUtils.requireNumber(arity, "arity");
        zkKitUtils.requireObject(leaves, "leaves");
     ...
     */
    indexOf(leaf) {
        zkKitUtils.requireTypes(leaf, "leaf", ["number", "string", "bigint"]);
    ...
    static verifyProof(proof, hash) {
        zkKitUtils.requireObject(proof, "proof");
        zkKitUtils.requireTypes(proof.root, "proof.root", ["number", "string", "bigint"]);
        zkKitUtils.requireTypes(proof.leaf, "proof.leaf", ["number", "string", "bigint"]);
        zkKitUtils.requireArray(proof.siblings, "proof.siblings");
        zkKitUtils.requireArray(proof.pathIndices, "proof.pathIndices");
        let node = proof.leaf;
