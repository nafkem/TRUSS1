...
function getPackedForbiddenCountries(forbiddenCountriesList) {
    if (forbiddenCountriesList.length > constants_js_1.MAX_FORBIDDEN_COUNTRIES_LIST_LENGTH) {
        throw new Error(`Countries list must be less than or equal to ${constants_js_1.MAX_FORBIDDEN_COUNTRIES_LIST_LENGTH}`);
    }
    // Step 1: Pad the list to the maximum length
    const paddedCountries = [...forbiddenCountriesList];
    while (paddedCountries.length < constants_js_1.MAX_FORBIDDEN_COUNTRIES_LIST_LENGTH) {
