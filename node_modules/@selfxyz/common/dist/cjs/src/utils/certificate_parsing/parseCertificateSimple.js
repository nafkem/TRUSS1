...
const elliptic_js_1 = require("./elliptic.js");
function parseCertificateSimple(pem) {
    ...
    try {
        ...
        const publicKeyAlgoFN = (0, oids_js_1.getFriendlyName)(publicKeyAlgoOID);
        const signatureAlgoOID = cert.signatureAlgorithm.algorithmId;
        const signatureAlgoFN = (0, oids_js_1.getFriendlyName)(signatureAlgoOID);
        ...
        certificateData.issuer = (0, utils_js_1.getIssuerCountryCode)(cert);
        certificateData.validity = {
            notBefore: cert.notBefore.value.toString(),
            notAfter: cert.notAfter.value.toString(),
        };
        const ski = (0, utils_js_1.getSubjectKeyIdentifier)(cert);
        ...
        if (certificateData.signatureAlgorithm === 'rsapss' &&
...
function getParamsRSAPSS(cert) {
    ...
    const hashAlgorithm = (0, oids_js_1.getFriendlyName)(pssParams.hashAlgorithm.algorithmId);
    const mgf = (0, oids_js_1.getFriendlyName)(pssParams.maskGenAlgorithm.algorithmId);
    return {
        modulus: modulusHex,
        exponent: exponentDecimal,
        bits: actualBits.toString(),
...
function getParamsECDSA(cert) {
    try {
        ...
        if (algorithmParams instanceof asn1js.ObjectIdentifier) {
            ...
            curveName = (0, oids_js_1.getSecpFromNist)((0, oids_js_1.getFriendlyName)(curveOid)) || 'Unknown';
            bits = (0, curves_js_1.getECDSACurveBits)(curveName);
        }
        // If the OID of the curve is not present, we try to get the curve parameters and identify the curve from them
            ...
            if (valueBlock.value && valueBlock.value.length >= 5) {
                ...
                const identifiedCurve = (0, curves_js_1.identifyCurve)(curveParams);
        ...
        if (publicKeyBuffer && curveName !== 'Unknown') {
            const elliptic = (0, elliptic_js_1.initElliptic)();
            const ec = new elliptic.ec((0, curves_js_1.getCurveForElliptic)(curveName));
...
const getCircuitName = (circuitMode, signatureAlgorithm, hashFunction, domainParameter, keyLength) => {
    const circuit = constants_js_1.circuitNameFromMode[circuitMode];
