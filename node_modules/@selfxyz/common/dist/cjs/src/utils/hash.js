...
exports.calculateUserIdentifierHash = calculateUserIdentifierHash;
exports.getSolidityPackedUserContextData = getSolidityPackedUserContextData;
...
function hash(hashFunction, bytesArray, format = 'bytes') {
    ...
    if (format === 'bytes') {
        return (0, bytes_js_1.hexToSignedBytes)(hashResult);
    }
    const actualForgeUtil = forge.util ? forge.util : forge.default.util;
    if (format === 'binary') {
        return actualForgeUtil.binary.raw.encode(new Uint8Array((0, bytes_js_1.hexToSignedBytes)(hashResult)));
    }
...
function packBytesAndPoseidon(unpacked) {
    const packed = (0, bytes_js_1.packBytesArray)(unpacked);
...
function calculateUserIdentifierHash(destChainID, userID, userDefinedData) {
    ...
    const sha256Hash = ethers_1.ethers.sha256(inputBytes);
    const ripemdHash = ethers_1.ethers.ripemd160(sha256Hash);
...
function getSolidityPackedUserContextData(destChainID, userID, userDefinedData) {
    ...
    return ethers_1.ethers.solidityPacked(['bytes32', 'bytes32', 'bytes'], [
        ethers_1.ethers.zeroPadValue(ethers_1.ethers.toBeHex(destChainID), 32),
        ethers_1.ethers.zeroPadValue('0x' + userIdHex, 32),
        ethers_1.ethers.toUtf8Bytes(userDefinedData),
    ]);
}
