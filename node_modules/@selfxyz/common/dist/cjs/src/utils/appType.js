...
class SelfAppBuilder {
    constructor(config) {
        ...
        const formattedEndpoint = (0, scope_js_1.formatEndpoint)(config.endpoint);
        ...
        if (!(0, uuid_js_1.validateUserId)(config.userId, config.userIdType ?? 'uuid')) {
...
function getUniversalLink(selfApp) {
    return `${constants_js_1.REDIRECT_URL}?selfApp=${encodeURIComponent(JSON.stringify(selfApp))}`;
}
