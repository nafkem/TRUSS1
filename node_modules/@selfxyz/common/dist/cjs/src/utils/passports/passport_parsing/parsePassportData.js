...
const brutForcePassportSignature_js_1 = require("./brutForcePassportSignature.js");
const parseDscCertificateData_js_1 = require("./parseDscCertificateData.js");
function findHashSizeOfEContent(eContent, signedAttr) {
    for (const hashFunction of constants_js_1.hashAlgos) {
        const hashValue = (0, hash_js_1.hash)(hashFunction, eContent);
        const hashOffset = (0, arrays_js_1.findSubarrayIndex)(signedAttr, hashValue);
        if (hashOffset !== -1) {
...
function findDG1HashInEContent(mrz, eContent) {
    ...
    for (const hashFunction of constants_js_1.hashAlgos) {
        const hashValue = (0, hash_js_1.hash)(hashFunction, formattedMrz);
        const normalizedHash = hashValue.map((byte) => (byte > 127 ? byte - 256 : byte));
        const hashOffset = (0, arrays_js_1.findSubarrayIndex)(eContent, normalizedHash);
        if (hashOffset !== -1) {
...
function getDgPaddingBytes(passportData, dg1HashFunction) {
    const formattedMrz = (0, format_js_1.formatMrz)(passportData.mrz);
    const hashValue = (0, hash_js_1.hash)(dg1HashFunction, formattedMrz);
    const normalizedHash = hashValue.map((byte) => (byte > 127 ? byte - 256 : byte));
    const dg1HashOffset = (0, arrays_js_1.findSubarrayIndex)(passportData.eContent, normalizedHash);
    const dg2Hash = passportData.dg2Hash;
    const normalizedDg2Hash = dg2Hash.map((byte) => (byte > 127 ? byte - 256 : byte));
    const dg2HashOffset = (0, arrays_js_1.findSubarrayIndex)(passportData.eContent, normalizedDg2Hash);
    return dg2HashOffset - dg1HashOffset - (0, hash_js_1.getHashLen)(dg1HashFunction);
}
...
function parsePassportData(passportData, skiPem = null) {
    ...
    const brutForcedPublicKeyDetails = (0, brutForcePassportSignature_js_1.brutforceSignatureAlgorithm)(passportData);
    let parsedDsc = null;
    let dscSignatureAlgorithmBits = 0;
    let dscMetaData;
    if (passportData.dsc) {
        parsedDsc = (0, parseCertificateSimple_js_1.parseCertificateSimple)(passportData.dsc);
        dscSignatureAlgorithmBits = parseInt(parsedDsc.publicKeyDetails?.bits || '0');
        dscMetaData = (0, parseDscCertificateData_js_1.parseDscCertificateData)(parsedDsc, skiPem);
    }
    return {
