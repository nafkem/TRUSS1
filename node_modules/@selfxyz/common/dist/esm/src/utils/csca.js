...
export function findStartIndexEC(point, messagePadded) {
    const pointNumArray = [];
...
export function findStartIndex(modulus, messagePaddedNumber) {
...
export function findOIDPosition(oid, message) {
...
export function getCSCAFromSKI(ski, skiPem = null) {
    const normalizedSki = ski.replace(/\s+/g, '').toLowerCase();
    const isSkiProvided = skiPem !== null;
    console.log('SKI-PEM provided');
    const cscaPemPROD = SKI_PEM[normalizedSki];
    const cscaPemDEV = SKI_PEM_DEV[normalizedSki];
    let cscaPem = null;
    if (isSkiProvided) {
...
export async function getSKIPEM(environment) {
