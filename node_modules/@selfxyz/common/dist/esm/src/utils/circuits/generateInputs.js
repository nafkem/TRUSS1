import { MAX_PADDED_ECONTENT_LEN, MAX_PADDED_SIGNED_ATTR_LEN, max_csca_bytes, COMMITMENT_TREE_DEPTH, OFAC_TREE_LEVELS, } from '../../constants/constants.js';
...
import { extractSignatureFromDSC, findStartPubKeyIndex, formatSignatureDSCCircuit, generateCommitment, getCertificatePubKey, getPassportSignatureInfos, pad, padWithZeroes, } from '../passports/passport.js';
...
export function generateCircuitInputsDSC(passportData, serializedCscaTree) {
    ...
    const [dscTbsBytesPadded, dscTbsBytesLen] = pad(passportMetadata.cscaHashFunction)(dscTbsBytes, max_dsc_bytes);
    ...
    const csca_pubKey_formatted = getCertificatePubKey(cscaParsed, passportMetadata.cscaSignatureAlgorithm, passportMetadata.cscaHashFunction);
    const signatureRaw = extractSignatureFromDSC(raw_dsc);
    const signature = formatSignatureDSCCircuit(passportMetadata.cscaSignatureAlgorithm, passportMetadata.cscaHashFunction, cscaParsed, signatureRaw);
...
export function generateCircuitInputsRegister(secret, passportData, serializedDscTree) {
    ...
    if (eContent.length > MAX_PADDED_ECONTENT_LEN[signatureAlgorithmFullName]) {
        console.error(`eContent too long (${eContent.length} bytes). Max length is ${MAX_PADDED_ECONTENT_LEN[signatureAlgorithmFullName]} bytes.`);
...
export function generateCircuitInputsVCandDisclose(secret, attestation_id, passportData, scope, selector_dg1, selector_older_than, merkletree, majority, passportNo_smt, nameAndDob_smt, nameAndYob_smt, selector_ofac, forbidden_countries_list, user_identifier) {
    ...
    const eContent_packed_hash = packBytesAndPoseidon(eContent_shaBytes.map((byte) => byte & 0xff));
...
export function generateCircuitInputsOfac(passportData, sparsemerkletree, proofLevel) {
...
export function generateCircuitInputsCountryVerifier(passportData, sparsemerkletree) {
...
export function findIndexInTree(tree, commitment) {
