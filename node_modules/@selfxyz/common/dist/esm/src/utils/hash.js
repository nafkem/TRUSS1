...
export function hash(hashFunction, bytesArray, format = 'bytes') {
...
export function getHashLen(hashFunction) {
    switch (hashFunction) {
        case 'sha1':
            return 20;
        case 'sha224':
...
export function customHasher(pubKeyFormatted) {
...
export function packBytesAndPoseidon(unpacked) {
    const packed = packBytesArray(unpacked);
    return customHasher(packed.map(String)).toString();
}
export function calculateUserIdentifierHash(destChainID, userID, userDefinedData) {
...
export function getSolidityPackedUserContextData(destChainID, userID, userDefinedData) {
    const userIdHex = userID.replace(/-/g, '');
    return ethers.solidityPacked(['bytes32', 'bytes32', 'bytes'], [
