...
import { k_csca, k_dsc, k_dsc_3072, k_dsc_4096, k_dsc_ecdsa, n_csca, n_dsc, n_dsc_3072, n_dsc_4096, n_dsc_ecdsa, } from '../../constants/constants.js';
import { bytesToBigDecimal, hexToDecimal, splitToWords } from '../bytes.js';
import { getCertificateFromPem, parseCertificateSimple, } from '../certificate_parsing/parseCertificateSimple.js';
...
export function initPassportDataParsing(passportData, skiPem = null) {
...
export function generateCommitment(secret, attestation_id, passportData) {
...
export function generateNullifier(passportData) {
    const signedAttr_shaBytes = hash(passportData.passportMetadata.signedAttrHashFunction, Array.from(passportData.signedAttr), 'bytes');
    const signedAttr_packed_hash = packBytesAndPoseidon(signedAttr_shaBytes.map((byte) => byte & 0xff));
    return signedAttr_packed_hash;
}
export function pad(hashFunction) {
    return hashFunction === 'sha1' || hashFunction === 'sha224' || hashFunction === 'sha256'
        ? shaPad
        : sha384_512Pad;
}
export function padWithZeroes(bytes, length) {
...
export function getPassportSignatureInfos(passportData) {
    const passportMetadata = passportData.passportMetadata;
...
export function getCertificatePubKey(certificateData, signatureAlgorithm, hashFunction) {
...
export function formatCertificatePubKeyDSC(certificateData, signatureAlgorithm) {
    const { publicKeyDetails } = certificateData;
    if (signatureAlgorithm === 'ecdsa') {
        const { x, y } = publicKeyDetails;
...
export function extractSignatureFromDSC(dscCertificate) {
...
export function formatSignatureDSCCircuit(cscaSignatureAlgorithm, cscaHashFunction, cscaCertificateData, signature) {
...
export function findStartPubKeyIndex(certificateData, rawCert, signatureAlgorithm) {
    const { publicKeyDetails } = certificateData;
    if (signatureAlgorithm === 'ecdsa') {
        const { x, y } = publicKeyDetails;
...
/// @dev valid for both DSC and CSCA
export function getSignatureAlgorithmFullName(certificateData, signatureAlgorithm, hashAlgorithm) {
    const { publicKeyDetails } = certificateData;
    if (signatureAlgorithm === 'ecdsa') {
...
export function extractRSFromSignature(signatureBytes) {
...
export function getNAndK(sigAlg) {
...
export function getNAndKCSCA(sigAlg) {
