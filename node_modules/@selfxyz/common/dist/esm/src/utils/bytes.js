...
export function computeIntChunkLength(byteLength) {
...
export function packBytesArray(unpacked) {
    const packSize = MAX_BYTES_IN_FIELD;
...
export function splitToWords(number, wordsize, numberElement) {
    let t = number;
    const words = [];
...
export function bytesToBigDecimal(arr) {
    let result = BigInt(0);
...
export function hexToSignedBytes(hexString) {
    let bytes = [];
...
export function toUnsignedByte(signedByte) {
    return signedByte < 0 ? signedByte + 256 : signedByte;
}
export function bigIntToChunkedBytes(num, bytesPerChunk, numChunks) {
    const res = [];
...
export function hexStringToSignedIntArray(hexString) {
    let result = [];
...
export function num2Bits(n, inValue) {
...
export function derToBytes(derValue) {
    const bytes = [];
