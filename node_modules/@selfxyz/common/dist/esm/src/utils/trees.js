...
import { CSCA_TREE_DEPTH, DSC_TREE_DEPTH, max_csca_bytes, OFAC_TREE_LEVELS, } from '../constants/constants.js';
...
export function getLeaf(parsed, type) {
    if (type === 'dsc') {
        // for now, we pad it for sha
        const tbsArray = Object.keys(parsed.tbsBytes).map((key) => parsed.tbsBytes[key]);
        const [paddedTbsBytes, tbsBytesPaddedLength] = pad(parsed.hashAlgorithm)(tbsArray, max_dsc_bytes);
...
export function getLeafDscTreeFromDscCertificateMetadata(dscParsed, dscMetaData) {
...
export function getLeafDscTreeFromParsedDsc(dscParsed) {
    return getLeafDscTreeFromDscCertificateMetadata(dscParsed, parseDscCertificateData(dscParsed));
}
export function getLeafDscTree(dsc_parsed, csca_parsed) {
    const dscLeaf = getLeaf(dsc_parsed, 'dsc');
    const cscaLeaf = getLeaf(csca_parsed, 'csca');
    return poseidon2([dscLeaf, cscaLeaf]).toString();
}
export function getLeafCscaTree(csca_parsed) {
    return getLeaf(csca_parsed, 'csca');
}
export function getDscTreeInclusionProof(leaf, serialized_dsc_tree) {
    const hashFunction = (a, b) => poseidon2([a, b]);
...
export function getCscaTreeInclusionProof(leaf, _serialized_csca_tree) {
...
export function getCscaTreeRoot(serialized_csca_tree) {
    let tree = new IMT(poseidon2, CSCA_TREE_DEPTH, 0, 2);
...
export function generateSMTProof(smt, leaf) {
    ...
    siblings.reverse();
    while (siblings.length < OFAC_TREE_LEVELS)
...
export function generateMerkleProof(imt, _index, maxleaf_depth) {
...
export function buildSMT(field, treetype) {
    let count = 0;
...
function processPassportNoAndNationality(passno, nationality, index) {
    ...
    const leaf = getPassportNumberAndNationalityLeaf(stringToAsciiBigIntArray(passno), stringToAsciiBigIntArray(countryCode), index);
    if (!leaf) {
...
export function getCountryLeaf(country_by, country_to, i) {
...
export function getPassportNumberAndNationalityLeaf(passport, nationality, i) {
...
export function getDobLeaf(dobMrz, i) {
    if (dobMrz.length !== 6) {
